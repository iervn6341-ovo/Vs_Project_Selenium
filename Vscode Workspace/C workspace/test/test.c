...
int main(){
	...
	printf("第一個多項式:");
	strcpy(input1,"2*x^4+2*x^3+3*x^2+7*x^1+1");
	...
	while(1){
		...
		if(input1[count] == '*'){
			point_list -> inte = input1[count-1];
		}
		if(input1[count] == '^'){
			point_list -> exp = input1[count+1];
		}
		count ++;
	...
	while(1){
		...
		if(input2[count] == '*' ){
			if(input2[count-1] != point_list -> inte){
				printf("輸入與第一個多項式不同的係數，請重新執行程式 !");
				return -1;
			}
			point_list -> inte = input2[count-1];
		}
		if(input2[count] == '^'){
			if(input2[count-1] != point_list -> exp){
				printf("輸入與第一個多項式不同的指數，請重新執行程式 !");
				return -1;
			}
			point_list -> exp = input2[count+1];
		}
		count ++;
		point_list = point_list -> next;
	}
